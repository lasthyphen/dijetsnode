...
# e.g.,
# ./scripts/build.sh
# ./scripts/tests.e2e.sh ./build/dijetsnode
# ENABLE_WHITELIST_VTX_TESTS=true ./scripts/tests.e2e.sh ./build/dijetsnode
...
DIJETSNODE_PATH="${1-}"
if [[ -z "${DIJETSNODE_PATH}" ]]; then
  echo "Missing DIJETSNODE_PATH argument!"
  echo "Usage: ${0} [DIJETSNODE_PATH]" >> /dev/stderr
  exit 255
fi
...
#################################
echo "running e2e tests against the local cluster with ${DIJETSNODE_PATH}"
./tests/e2e/e2e.test \
--ginkgo.v \
--log-level debug \
--network-runner-grpc-endpoint="0.0.0.0:12342" \
--network-runner-avalanchego-path=${DIJETSNODE_PATH} \
