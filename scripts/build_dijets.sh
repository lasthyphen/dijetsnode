...
if version_lt "$(go_version)" "$go_version_minimum"; then
    echo "DijetsNode requires Go >= $go_version_minimum, Go $(go_version) found." >&2
    exit 1
fi
...
build_args="$race"
echo "Building DijetsNode..."
go build $build_args -ldflags "-X github.com/lasthyphen/dijetsnode/version.GitCommit=$git_commit $static_ld_flags" -o "$dijetsnode_path" "$DIJETS_PATH/main/"*.go
