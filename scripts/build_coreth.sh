...
if [[ ! -d "$coreth_path" ]]; then
  go get "github.com/lasthyphen/utilitychain@$coreth_version"
fi
...
cd "$coreth_path"
go build $build_args -ldflags "-X github.com/lasthyphen/utilitychain/plugin/evm.Version=$coreth_version $static_ld_flags" -o "$evm_path" "plugin/"*.go
cd "$DIJETS_PATH"
